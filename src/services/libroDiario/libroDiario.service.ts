...
export class LibroDiarioService {
  ...
  public static async obtenerLibroDiarioPorRangoFechas(
    fechaInicio: string,
    fechaFin: string
  ): Promise<LibroDiario> {
    try {
      const { data } = await api.get<LibroDiario>(
        "libro-diario/asientos/rango-fechas",
        {
          params: {
            fechaInicio,
            fechaFin,
      ...
      if (axios.isAxiosError(error)) {
        const message =
          ...
          (error.response?.data?.message || error.message);
        throw new Error(
          `Error al obtener libro diario por rango de fechas: ${message}`
        );
      }
      throw new Error(
        "Error desconocido al obtener libro diario por rango de fechas"
      );
    }
  }

  public static async buscarAsientosPorDescripcion(
    descripcion: string
  ): Promise<LibroDiario> {
    try {
      const { data } = await api.get<LibroDiario>(
        "libro-diario/asientos/search",
        {
          params: { descripcion },
        }
      );
