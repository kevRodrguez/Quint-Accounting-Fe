...
export function ChartAreaInteractive() {
  ...
  const obtenerAsientosUltimosTresMeses = async () => {
    ...
    try {
      const response =
        await EstadisticasService.totalAsientosUltimosTresMeses();
      setAsientosUltimosTresMeses(response);
    } catch (error) {
      toast.error(
        "Error al obtener los asientos de los últimos tres meses. Por favor, intenta de nuevo."
      );
    }
    setIsLoading(false);
  };
  ...
  const filteredData = React.useMemo(() => {
    ...
    const dates = asientosUltimosTresMeses.map(
      (item: { fecha: string | number | Date }) => new Date(item.fecha)
    );
    const maxDate = new Date(
      Math.max(...dates.map((d: { getTime: () => any }) => d.getTime()))
    );
